...
esp_err_t WebServerManager::handleCoreDumpInfo(httpd_req_t *req) {
  ...
  if (available) {
    ...
    // Integrity is reported separately: a truncated dump is still worth
    // downloading, and the partition here is deliberately small.
    cJSON_AddBoolToObject(root, "valid", esp_core_dump_image_check() == ESP_OK);
