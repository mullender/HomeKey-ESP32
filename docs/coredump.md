...
survives the reboot with it intact. The web UI decodes it on-device, so the
...
```bash
curl -s http://<device>/coredump/info | jq
```
...
```bash
curl -o coredump.elf http://<device>/coredump
...
```bash
curl -X POST http://<device>/coredump/erase
```

## Caveats

- The partition is 128 KB with `CONFIG_ESP_COREDUMP_MAX_TASKS_NUM` at 16, which
  is comfortable for this firmware. A dump that still overruns is truncated and
  `valid` reports `false`; the summary is written first, so task and PC remain
  readable.
