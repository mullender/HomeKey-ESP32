<script lang="ts">
  ...
  import { getBootLog, clearBootLog } from '$lib/services/api';
  ...
  async function exportLogs(format = 'json') {
    ...
    // Pull the device-side buffer too. The live stream cannot contain the early
    // boot sequence -- including the reset reason -- so an export without it is
    // missing exactly the part that explains an unexpected restart.
    const bootLog = await getBootLog();
        ...
        <div class="min-h-0">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pt-3 mt-2 border-t border-base-300/50 sm:border-t-0 sm:pt-0 sm:mt-0">
            ...
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <button onclick={loadBootLog} class="btn btn-sm btn-outline gap-1" disabled={bootLogLoading}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
                </svg>
                {bootLogLoading ? 'Loading…' : 'Boot log'}
              </button>
