...
TEST(build_state_packet_matches_wire_format) {
    ...
    // IMPROV(6) + ver(1) + type(1) + len(1) + data(1) + checksum(1) + newline(1) = 12
    // The trailing 0x0A is a framing footer; it is NOT counted in the length
    // byte at offset 8 and NOT included in the checksum sum at offset 10.
    CHECK_EQ(pkt.size(), 12u);
    CHECK_EQ(pkt[0], 'I');
    CHECK_EQ(pkt[5], 'V');
    CHECK_EQ(pkt[6], 1);            // version
    CHECK_EQ(pkt[7], 0x01);         // TYPE_CURRENT_STATE
    CHECK_EQ(pkt[8], 1);            // data length (excludes newline)
    CHECK_EQ(pkt[9], 0x02);         // STATE_AUTHORIZED
    CHECK_EQ(pkt[10], sum8(std::vector<uint8_t>(pkt.begin(), pkt.begin() + 10)));
    CHECK_EQ(pkt[11], 0x0A);        // trailing newline
}
...
TEST(build_frame_rejects_data_over_255_bytes) {
    ...
    CHECK(out.empty());
    // Exactly 255 must still succeed. Total = 6+1+1+1 (header) + 255 (data)
    // + 1 (checksum) + 1 (newline) = 265 bytes on the wire.
    std::vector<uint8_t> at_limit(255, 0xCD);
    auto ok = improv::buildFrame(PacketType::Rpc, at_limit);
    CHECK(!ok.empty());
    CHECK_EQ(ok[8], 255);
    CHECK_EQ(ok.size(), 11u + 255u);
    CHECK_EQ(ok.back(), 0x0A);
}
