...
std::vector<std::vector<uint8_t>> splitFrames(const std::vector<uint8_t> &buf) {
    ...
    size_t i = 0;
    // Frame layout: IMPROV(6) + ver(1) + type(1) + len(1) + data + checksum(1)
    //             + newline(1) -- so header size is 9, footer is 2 (chk+lf).
    while (i + 11 <= buf.size()) {
        if (std::memcmp(&buf[i], "IMPROV", 6) != 0) { i++; continue; }
        if (buf[i + 6] != 1) { i++; continue; }
        uint8_t data_len = buf[i + 8];
        size_t total = 11 + data_len;  // 9 header + data + checksum + newline
