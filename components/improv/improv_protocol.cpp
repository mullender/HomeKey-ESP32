...
std::vector<uint8_t> buildFrame(PacketType type, const std::vector<uint8_t> &data) {
    ...
    // Wire layout:
    //   6 preamble + 1 version + 1 type + 1 length + data + 1 checksum + 1 newline
    //
    // The trailing 0x0A newline is a framing footer, NOT counted in the
    // length header and NOT covered by the checksum sum. It matches both
    // ESPHome's tx footer and improv-wifi/sdk-serial-js `writePacketToStream`
    // (serial.ts:689-695) and provides a resynchronization delimiter after
    // each device frame: if the client parser is stuck in its "discard until
    // \n" state when we start writing, the LF we emit after the swallowed
    // frame lets subsequent frames parse cleanly. It does NOT protect any
    // frame whose preamble arrives while the parser is still discarding
    // (that frame is lost); it recovers the ones that follow.
    std::vector<uint8_t> out;
    out.reserve(11 + data.size());
